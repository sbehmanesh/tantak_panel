  ...
  <v-menu offset-y bottom v-model="menu">
    <template v-slot:activator="{ on, attrs }">
      <div class="center-div">
        <span
          class="white--text px-2 font_15 font_bold"
          v-if="$vuetify.breakpoint.mdAndUp"
          >{{ fullname }}</span
        >
        <div class="mr-2">
          <v-badge
          v-if="!Boolean($store.state.ws.is_connect)"
            width="12"
            bordered
            height="12"
            icon="priority_high"
    ...
    <v-card width="250" class="pa-2">
      <div class="pa-3 primary">
        <v-icon class="pointer" size="22" color="#fff" @click="openSetting()"
          >settings</v-icon
        >
      </div>
