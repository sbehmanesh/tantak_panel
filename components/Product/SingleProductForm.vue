<template>
  <div>
    ...
    <v-row class="pa-2 mt-10">
      ...
      <v-col cols="12" md="1" class="text-center"> کد </v-col>
      <v-col cols="12" md="1" class="text-center"> موجودی </v-col>
      <v-col cols="12" md="1" class="text-center"> وزن بسته gr</v-col>
      <v-col cols="12" md="3" class="text-center"> عملیات </v-col>
    </v-row>

    <v-col cols="12" v-if="product.single_sell_variation_combinations.length == 0">
    ...
    <v-row
      v-for="(sv, index) in product.single_sell_variation_combinations"
      :key="'sv' + index"
      :class="index % 2 == 0 ? 'odd-row' : ''"
    >
      <v-col cols="12" md="2" class="text-center">
        <span v-if="sv.variation1_info && sv.variation1_info.variation_type"> {{ sv.variation1_info.variation_type.value }} {{ sv.variation1_info.value }} </span>
        <span v-if="sv.variation2_info && sv.variation2_info.variation_type"> / {{ sv.variation2_info.variation_type.value }} {{ sv.variation2_info.value }} </span>
        <span v-if="sv.variation3_info && sv.variation3_info.variation_type"> / {{ sv.variation3_info.variation_type.value }} {{ sv.variation3_info.value }} </span>
      </v-col>
      <v-col cols="12" md="1" class="text-center"> <amp-input is-price v-model="sv.price" /></v-col>
...
export default {
  ...
  methods: {
    ...
    update(index) {
      this.loading = true
      let form = this.product.single_sell_variation_combinations[index]
    ...
    deleteItem() {
      this.loading = true
      let selected_id = this.product.single_sell_variation_combinations[this.selected_item].id
