<template>
  <div>
    <BaseTable url="/product-variation-combination" :headers="headers" :BTNactions="btn_actions"> </BaseTable>

    <v-dialog v-model="updateeDiaolog" :width="$vuetify.breakpoint.mdAndUp ? 1000 : 1000">
      <v-card class="pa-5">
        ...
        <v-form v-model="valid" @submit.prevent="submit()">
          <v-row class="pa-3">
            ...
            <v-col cols="4">
              <amp-input is-number text="وزن بسته کیلوگرم" v-model="form.weight" rules="require" />
            </v-col>
            <v-col cols="4" v-if="form.type == 'single_sell'">
              <amp-input text="موجودی" is-number v-model="form.max" rules="require" />
            </v-col>
            <v-col cols="6">
              <amp-button
                text="به روزرسانی"
    ...
    <v-dialog width="800" v-model="history_dialog.show">
      <v-card v-if="history_dialog.show" class="pa-5">
        <StockHistory @closeDialog="closeDialog" v-model="history_dialog.item.id" />
      </v-card>
    </v-dialog>
  </div>
</template>
      ...
      { text: 'محصول', value: 'product_name' },
      {
        ...
        value: (body) => {
          ...
          if (body.variation1_info) {
            name += body.variation1_info.variation_type.value + ' ' + body.variation1_info.value
          }
          if (body.variation2_info) {
            name += ' - ' + body.variation2_info.variation_type.value + ' ' + body.variation2_info.value
          }
          if (body.variation3_info) {
            name += ' - ' + body.variation3_info.variation_type.value + ' ' + body.variation3_info.value
          }
          return name
        },
        disableSort: 'true',
